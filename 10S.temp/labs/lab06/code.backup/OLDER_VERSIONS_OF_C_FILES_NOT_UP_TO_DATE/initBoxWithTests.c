...
int testBoxesApproxEqual()
{
  ...
  failures += assertTrue("boxesApproxEqual(b0,b0,tol)", boxesApproxEqual(b0,b0,tol));

  // Now, check that b1 is not equal to b0, both ways:

  failures += assertTrue("!boxesApproxEqual(b1,b0,tol)", !boxesApproxEqual(b1,b0,tol));
  failures += assertTrue("!boxesApproxEqual(b1,b0,tol)", !boxesApproxEqual(b1,b0,tol));
  
  // Check all the others (one way is probaby enough)

  failures += assertTrue("!boxesApproxEqual(b2,b0,tol)", !boxesApproxEqual(b2,b0,tol));
  failures += assertTrue("!boxesApproxEqual(b3,b0,tol)", !boxesApproxEqual(b3,b0,tol));
  failures += assertTrue("!boxesApproxEqual(b4,b0,tol)", !boxesApproxEqual(b4,b0,tol));
  
  // Now check that b5 and b6 are equal in both directions

  failures += assertTrue("boxesApproxEqual(b5,b6,tol)", boxesApproxEqual(b5,b6,tol));
  failures += assertTrue("boxesApproxEqual(b6,b5,tol)", boxesApproxEqual(b6,b5,tol));
...
int testInitBox()
{
  ...
  failures += assertTrue("b1", boxesApproxEqual(b1,b1Expected,tol));

  initBox(&b2, 9.8, 7.6, 5.4, 3.2); 
  b2Expected.ul.x = 9.8;  b2Expected.ul.y = 7.6;
  b2Expected.width = 5.4;  b2Expected.height = 3.2;
  failures += assertTrue("b2", boxesApproxEqual(b2,b2Expected,tol));
