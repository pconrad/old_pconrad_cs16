...
// find the smallest number in an array of numbers (double)
...
int testSmaller()
{
  ...
  failures += CHECK_EXPECT_DOUBLE(smaller(0.1, 0.2),0.1, tol);
  failures += CHECK_EXPECT_DOUBLE(smaller(1.0, 10.0),1.0, tol);
  failures += CHECK_EXPECT_DOUBLE(smaller(3.14, 3.0),3.0, tol);
  failures += CHECK_EXPECT_DOUBLE(smaller(-12.0, -11.5),-12.0, tol);
...
int testMinValue()
{
  ...
  failures += CHECK_EXPECT_DOUBLE(minValue(a, 1), 0.01, tol);
  failures += CHECK_EXPECT_DOUBLE(minValue(b, 3), 0.002, tol);
  failures += CHECK_EXPECT_DOUBLE(minValue(c, 4), -12.3, tol);
  failures += CHECK_EXPECT_DOUBLE(minValue(d, 4), 1.0001,tol); 
