...
int checkExpectIntArrayWithFileAndLine(char * file, 
			int line, 
			char * label, // the label e.g. "isE('f')"
			int * check, // the funciton call we are checking
			int * expect, // what we expect to get
  ...
  for (i=0; i<n; i++)
    {
      if (check[i] != expect[i])
	{
	  printf("TEST FAILED: %s line %d %s at element %d.... got %d expected %d\n", 
		 file, line, label, i, check[i], expect[i]);
	  return 1;
	}
