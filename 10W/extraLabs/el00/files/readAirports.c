...
int runTests()
{
  ...
  failures += assertTrue("airport1.code ", strcmp("ABC",airport1.code)==0 );
  failures += assertTrue("airport1.lat  ", approxEqual(123.45,airport1.lat, tol) );
  failures += assertTrue("airport1.lon  ", approxEqual(67.89,airport1.lon,tol) );
  failures += assertTrue("airport1.city ", strcmp("TestCity",airport1.city)==0 );
  failures += assertTrue("airport1.state", strcmp("CA",airport1.state)==0 );

  initAirportFromString(&airport2, testString2);

  //@@@ TODO: ADD TESTS FOR airport 2
  //@@@ You need five calls to assertTrue, similar to the ones above.
...
int indexOfAirportWithSmallestLongitude(struct Airport *airports, int airportCount)
{
  int smallestSoFar = 0;
  int i;
